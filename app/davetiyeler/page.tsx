...
const statusPresentation = {
  yes: { label: 'Katılıyor', className: 'bg-[#e3f1eb] text-[#286957]', icon: Check },
  no: { label: 'Katılamıyor', className: 'bg-[#f7e7e4] text-[#9b4650]', icon: X },
  maybe: { label: 'Henüz net değil', className: 'bg-[#efebf6] text-[#6d5a88]', icon: HelpCircle },
  awaiting: { label: 'Yanıt bekleniyor', className: 'bg-[#f3eee9] text-[#756862]', icon: Clock3 },
};
...
export default async function InvitationsPage() {
  ...
  const answered = metrics.attendingGuests + metrics.declinedGuests + metrics.maybeGuests;
    ...
    <main className="min-h-screen bg-[#f8f4ef] text-foreground lg:pl-[226px]">
      <aside className="fixed inset-y-0 left-0 hidden w-[226px] flex-col border-r border-[#eadfd8] bg-[#fffdfa] p-5 lg:flex">
        ...
        <nav className="mt-9 space-y-1 text-sm">
          ...
          <a href="#guests" className="nav-link"><Users />Davetliler</a>
          <a href="#" className="nav-link"><Settings />Ayarlar</a>
        </nav>
        <div className="mt-auto rounded-2xl bg-[#f5eee9] p-4"><p className="text-xs font-semibold">Düğüne 12 gün kaldı</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white"><div className="h-full w-[61%] rounded-full bg-primary" /></div><p className="mt-2 text-[10px] text-muted-foreground">Yanıt oranı %{metrics.invitedGuests ? Math.round(answered / metrics.invitedGuests * 100) : 0}</p></div>
      </aside>
      ...
      <div className="mx-auto max-w-[1320px] px-4 pb-16 pt-6 sm:px-7 lg:pt-8">
        <div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
          <div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">Davetiye yönetimi</p><h1 className="mt-2 font-heading text-[32px] font-semibold tracking-[-0.035em]">Elif & Arda</h1><div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground"><span className="flex items-center gap-1.5"><CalendarDays className="size-3.5" />12 Eylül 2026 · 19:30</span><span className="flex items-center gap-1.5"><MapPin className="size-3.5" />Liva Davet</span></div></div>
          <ShareActions slug="ahmet-zeynep-x7p92k" guests={guests.map((guest) => ({ name: guest.name, status: guest.status ?? 'awaiting', partySize: guest.partySize, note: guest.note }))} />
        </div>
        ...
        <section id="guests" className="mt-6 overflow-hidden rounded-[20px] border border-[#eadfd8] bg-[#fffdfa] shadow-[0_12px_36px_rgba(70,45,38,.05)]">
          <div className="flex flex-col gap-3 border-b border-[#eee5df] p-5 sm:flex-row sm:items-center sm:justify-between">
            <div><h2 className="font-heading text-xl font-semibold">Davetli yanıtları</h2><p className="mt-1 text-xs text-muted-foreground">Yanıtlar gönderildikçe bu liste otomatik güncellenir.</p></div>
            <div className="relative w-full sm:w-64"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input aria-label="Davetli ara" placeholder="İsimle ara" className="h-10 w-full rounded-xl border border-[#e4d9d3] bg-white pl-10 pr-3 text-xs outline-none focus:border-[#bd858f]" /></div>
          </div>
