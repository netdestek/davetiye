...
import { AdminVideoLibrary } from '@/components/admin-video-library';
...
const nav = [
  ...
  { label: 'Video yüklemeleri', icon: Film },
  ...
  return (
    <main className="min-h-screen bg-[#f5f6f8] text-[#25282b] lg:pl-[238px]">
      <aside className="fixed inset-y-0 left-0 hidden w-[238px] flex-col bg-[#182522] px-4 py-5 text-[#d8e2df] lg:flex">
        <a href="/" className="flex items-center gap-2.5 px-2"><span className="liquid-icon liquid-icon--brand size-9 rounded-xl"><Heart className="size-4 fill-current" /></span><span className="font-heading text-xl font-semibold text-white">davetly</span><Badge className="ml-auto bg-white/10 text-[9px] text-white">ADMIN</Badge></a>
        <nav className="mt-9 space-y-1" aria-label="Admin menüsü">
          {nav.map((item) => <a key={item.label} href="#" className={`flex h-10 items-center gap-3 rounded-xl px-3 text-[13px] font-medium transition ${item.active ? 'bg-white/10 text-white' : 'text-[#9fb0ab] hover:bg-white/5 hover:text-white'}`}><item.icon className="size-[17px]" />{item.label}</a>)}
        </nav>
      ...
      <header className="flex h-[70px] items-center border-b border-[#e2e5e8] bg-white px-4 sm:px-7">
        <div className="flex items-center gap-2 lg:hidden"><span className="liquid-icon liquid-icon--neutral size-9 rounded-xl"><ShieldCheck className="size-4" /></span><strong className="text-sm">Admin</strong></div>
      ...
      <div className="mx-auto max-w-[1440px] px-4 pb-14 pt-7 sm:px-7">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <AdminMetric label="Toplam kullanıcı" value="2.842" note="bu ay +184" icon={Users} tone="liquid-icon--blue" />
          <AdminMetric label="Yayındaki davetiye" value="1.126" note="bugün +23" icon={Heart} tone="liquid-icon--rose" />
          <AdminMetric label="Aylık gelir" value="₺184,6K" note="geçen aya göre %12" icon={CircleDollarSign} tone="liquid-icon--sage" />
          <AdminMetric label="Saklanan video" value="1.126" note="bugün +23 yükleme" icon={Film} tone="liquid-icon--gold" />
        </div>
...
function AdminMetric({ label, value, note, icon: Icon, tone }: { label: string; value: string; note: string; icon: typeof Users; tone: string }) {
  return <div className="rounded-2xl border border-[#e1e5e8] bg-white p-4 shadow-[0_8px_25px_rgba(27,37,34,.035)]"><div className="flex items-start justify-between"><span className={`liquid-icon size-9 rounded-xl ${tone}`}><Icon className="size-[17px]" /></span><Badge variant="secondary" className="bg-[#edf4f1] text-[9px] text-[#37725f]">Canlı</Badge></div><strong className="mt-4 block text-[24px] font-bold tracking-[-0.03em]">{value}</strong><p className="mt-1 text-[11px] font-semibold">{label}</p><p className="mt-1 text-[9px] text-[#8a9296]">{note}</p></div>;
}
